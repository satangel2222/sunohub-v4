                            ...
                            >
                                <div className="block relative mb-2">
                                    <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 relative">
                                        ...
                                        {!isManageMode && canDelete && (
                                            <button 
                                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); if(confirm(`彻底删除《${song.title}》吗？`)) deleteSong(song.id!).then(fetchSongs); }}
                                                className="absolute top-2 right-2 p-1.5 bg-red-500/80 hover:bg-red-600 text-white rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-200"
                                                title="快速删除"
                                            >
                                                <Trash2 size={14} />
                                            </button>
