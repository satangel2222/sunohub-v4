...
const Feed: React.FC = () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            alert("删除失败: " + err.message); // Keep alert for error redundancy for now, or could use a toast
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="animate-fade-in-up relative pb-20">
            ...
            {/* Replace the native confirm calling buttons with these handlers */}
            {/* In the single song card map: */}
            {/* onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteClick(song); }} */}

            {/* In the batch manage bar: */}
