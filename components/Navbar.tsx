...
const Navbar: React.FC<NavbarProps> = ({ onlineCount = 1 }) => {
  ...
  const handleBulkUpdate = async () => {
    console.log('[Admin] Bulk update clicked', { targetArtist, newArtist });
    if (!targetArtist.trim() || !newArtist.trim()) {
      alert("请输入原名称和新名称");
      return;
    }
    if (!confirm(`确定要将所有 "${targetArtist}" 替换为 "${newArtist}" 吗？此操作不可逆！`)) {
      console.log('[Admin] User cancelled update');
      return;
    }

    setAdminLoading(true);
    try {
      console.log('[Admin] Calling service...');
      const count = await bulkUpdateArtistName(targetArtist, newArtist);
      console.log('[Admin] Service returned count:', count);
